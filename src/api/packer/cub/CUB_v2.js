...
function debugRegion(region, color, solid, duration, checkered){
    let x = region.x, y = region.y, z = region.z, w = region.width, h = region.height, l = region.length;
    let debugUID = Debug.Viz.DrawVolume(x + w/2, y + h/2, z + l/2, w, h, l, color || 0xaaffff, duration || -1, !Boolean(solid), checkered);
    return debugUID;
}
...
class Region{
     ...
     */
    GetCorners(offset){
        let x = this.x - offset, y = this.y - offset, z = this.z - offset, w = this.width + offset * 2, h = this.height + offset * 2, l = this.length + offset * 2;
    ...
    GetFloorPoints(result){
        ...
        let x = this.x, y = this.y, z = this.z, w = this.width, l = this.length;
    ...
    ContainsRegion(offset, subRegion){
        let x = this.x - offset, y = this.y - offset, z = this.z - offset, w = this.width + offset * 2, h = this.height + offset * 2, l = this.length + offset * 2;
        let rx = subRegion.x, ry = subRegion.y, rz = subRegion.z, rw = subRegion.width, rh = subRegion.height, rl = subRegion.length;
    ...
    ConnectFloorRects(other){
        let ptsA = this.GetFloorPoints(tempPoints),
        ...
        let rectangles = rectanglesFromPoints(intersections);
...
class PackedContainer{
    ...
    CommitFit(item, containingRegion, placement, orientation){
        ...
        for(let iPacked = 0; iPacked < numPackedItems; iPacked++){
            ...
            if(intersects){
                console.log('CommitFit.intersects with packed item');
                return false;
            }
        }
    ...
    ProcessRegionsMergeExpand(){
        ...
        let levelsYCats = Object.keys(levels);
    ...
    ProcessRegionsForZeroRegions(){
        let regions = this.regions;
        let minRegionAxis = this.assistant.minRegionAxis;
    ...
    ProcessRegionsForPackedItems(harsh){
        let packedItems = this.packedItems,
...
class PackingAssistant{
    ...
    constructor(items, packedContainer){
        ...
        this.regions = this.packedContainer.regions;
        ...
        let minDimensionsSearchSet = this.workingItems;
        this.minDimensions = Item.GetMinDimensionsOverall(minDimensionsSearchSet);
    }
    ...
    RateFit(fit, newRegions){
        ...
        let minDimensionsNoWasteFactor = this.params.minDimensionsNoWasteFactor;
        ...
        let minZ_weight = this.packedContainer.params.minZ_weight;
        let minWaste_weight = this.packedContainer.params.minWaste_weight;
        let score = minZScore * minZ_weight + minWasteScore * minWaste_weight;
        return score;
    }
            ...
            if(region.volume > volumeItem){
                ...
                for(let iOrient = 0; iOrient < validOrientations.length; iOrient++){
                    ...
                    if(regionFitTest !== false){
                        ...
                        let newRegions = dummyRegion.Subtract(regionFitTest, this.minRegionAxis, this.params.skipTop);
                        if(newRegions === undefined) newRegions = [];
                        if(dummyRegion.length > this.minRegionAxis)
                            newRegions.push(dummyRegion);

                        let orientationScore = {
                            region: iRegion,
                            orientation: orientation, 
                            n: this.RateFit(item, regionFitTest, newRegions)
                        };
    ...
    FitLessWaste(item){
        ...
        let regionFitTest = region.FitTest(smallValue, dimensions[0], dimensions[1], dimensions[2]);
        if(regionFitTest !== false){
            return this.packedContainer.CommitFit(item, region, regionFitTest, orientation);
        }
