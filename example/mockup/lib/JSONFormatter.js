...
JSONFormatter = (function() {
  ...
  var keyStyle = function(key){
    return '<span style="color: #e7a700;">' + key + '</span>';
  }
  var tjvCount = 0;
  ...
  var init = function( json, options ) {
        ...
        if(typeof v3 == 'object') {
          ...
          $.each(v3, function(k4, v4) {
            if(typeof v4 == 'object' && v4 != null) {
              $('#' + settings.list_id + ' #' + ulId + '-' + k3).append('<li>' + keyStyle(k4) + ' <span>{</span> <ul id="'+k4+'-'+loopCount+'"></ul></li>');
              loopAgain(v4, k4, k4 + '-' + loopCount);
            }
            else {
              $('#' + settings.list_id + ' #' + ulId + '-' + k3).append('<li>' + keyStyle(k4) + ': ' + v4 + '</li>');
            }
      ...
      $.each(v, function(nextKey, nextVal) {
        ...
        if(nextVal != null && typeof nextVal == 'object') {
          if(nextVal.length == 0) {
            ...
            $('#' + settings.list_id + ' #' + ulId).append('<li><i>' + keyStyle(nextKey) + ':</i> []</li>');
          }
          else if(nextVal.length >= 1) {
            // an object of objects
            $('#' + settings.list_id + ' #' + ulId).append('<li><b>' + keyStyle(nextKey) + ':</b> <span>[</span> ' + newList + '</li>');
          ...
          // }
          // else {
            $('#' + settings.list_id + ' #' + ulId).append('<li><i>'+ keyStyle(nextKey) + ':</i> ' + nextVal + '</li>');
    ...
    $.each(json, function(key, val) {
      ...
      if(val != null && typeof val == 'object') {
        ...
        if(goObj) {
          $('#' + settings.list_id).append('<li><b>' + keyStyle(key) + ':</b> <span>[</span><ul id="' + nk + '-' + loopCount + '"></ul></li>');
          ...
          $('#' + settings.list_id).append('<li><b>' + keyStyle(key) + ':</b> <span>{</span><ul id="' + key + '-' + loopCount + '"></ul></li>');
          loopAgain(val, key, key + '-' + loopCount);
        }

      }
      else {
        $('#' + settings.list_id).append('<li><i>' + keyStyle(key) + ':</i> ' + val + '</li>');
      }
    });
  ...
  addToggles = function( listId ) {
    $('#' + listId + " > li").find('ul').each(function() {
      let i = tjvCount++;
      let id = $(this).parent().find('ul').addClass('collapse').attr('id');
      $(this).parent().find('span').eq(0).after('<span class="collapse-toggle" data-collapse href="#' + id + '"> - </span>');
    });
  };
